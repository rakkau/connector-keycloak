...
public class KeycloakAdminRESTUser implements KeycloakClient.User {
    ...
    public Uid createUser(KeycloakSchema schema, String realmName, Set<Attribute> createAttributes)
        ...
        if (newUser.getUsername() == null || newUser.getUsername().isBlank()) {
            throw new ConnectorException("Username is missing or blank.");
        }
        LOGGER.info("Converted attributes to UserRepresentation. Username: {0}, Email: {1}",
                newUser.getUsername(), newUser.getEmail());
        newUser.setEnabled(true); // Ensure the user is marked as active
...
/*        if (!clientRolesToAdd.isEmpty()) {
    ...
    private void updatePassword(String realmName, String userId, CredentialRepresentation credential, final Boolean permanent)
        ...
        try {
            ...
            ex.setAffectedAttributeNames(Collections.singletonList(OperationalAttributes.PASSWORD_NAME));
            throw ex;
        }
    }
